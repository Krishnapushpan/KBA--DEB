...
route.get('/listofcar', async(req,res)=>{
  ...
  // route.post('/renting', async (req, res) => {
  //   // try {
  //     const { username, firstname, lastname, mobileno, address, emailid, carname, carnumber, pickup, destination, pickupdate, pickuptime, returndate, returntime } = req.body;
  
  //     // Check if the car exists
  //     const car = await Adding.findOne({ carnumber });
  //     if (!car) {
  //       return res.status(404).json({ message: "No car with this number found" });
  //     }
  
  //     // Check if the car is already booked
  //     const existBooking = await Booking.findOne({ carnumber });
  //     if (existBooking) {
  //       return res.status(400).json({ message: "Car is not available" });
  //     }
  
  //     // Create a new booking
  //     const newBooking = new Booking({  username, firstname, lastname, mobileno, address, emailid, carname, carnumber, pickup, destination, pickupdate, pickuptime, returndate, returntime });
  //     await newBooking.save();
...
// const Booking = require('./models/Booking'); // Assuming your Booking model is in the models folder
